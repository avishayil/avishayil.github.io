...
const SNAPSHOT: Repo[] = [
  ...
  { name: 'home-cluster', description: 'My home Kubernetes (k3s) cluster managed by GitOps (Flux).', url: `https://github.com/${GITHUB_USER}/home-cluster`, language: 'Shell', stars: 39, topics: ['kubernetes', 'gitops', 'k3s'] },
...
export async function getFeaturedRepos(): Promise<Repo[]> {
  try {
    const featured = keepFeatured(await fetchRepos()).sort(byStarsDesc);
    if (featured.length === 0) return [...SNAPSHOT].sort(byStarsDesc);
    return attachDownloads(featured);
  } catch (err) {
    console.warn(
      `[github] live fetch failed, using snapshot: ${(err as Error).message}`
    );
    return [...SNAPSHOT].sort(byStarsDesc);
  }
}
...
export async function getAllRepos(): Promise<Repo[]> {
  try {
    ...
    if (repos.length === 0) return [...SNAPSHOT].sort(byStarsDesc);
